...
def _mineru_paths(book: BookRecord, cfg: dict[str, Any]) -> tuple[Path, Path, Path, Path]:
    output_dir = Path(book.output_dir)
    raw_path = output_dir / cfg["paths"]["mineru_raw"].format(book_id=book.book_id)
    parsed_path = output_dir / cfg["paths"]["mineru_parsed"].format(book_id=book.book_id)
    image_map_path = output_dir / cfg["paths"]["image_map"]
    status_path = output_dir / cfg["paths"].get("mineru_status", "mineru/status/{book_id}.json").format(book_id=book.book_id)
...
def _validate_mineru_result(
    ...
    status = _build_mineru_status(book, content, image_map, cfg, source)
    if not status["valid"]:
        raise MinerUParseIncompleteError("; ".join(status["errors"]))
    return status


def mineru_cache_is_valid(book: BookRecord, cfg: dict[str, Any]) -> bool:
    raw_path, parsed_path, image_map_path, status_path = _mineru_paths(book, cfg)
    cache_only = bool(cfg["runtime"].get("cache_only"))
    if cache_only and not raw_path.is_file():
        return False
    ...
    if not isinstance(image_map, dict):
        return False
    status = _build_mineru_status(book, content, image_map, cfg, "cache")
    try:
...
def _mineru_slot_dir(cfg: dict[str, Any], book: BookRecord) -> Path:
    directory = runtime_dir(cfg, "mineru_slots", fallback=Path(book.output_dir).parent)
...
def _try_take_slot(slot_dir: Path, provider: MinerUProvider, book: BookRecord, stale_seconds: float, now: float) -> Path | None:
...
def _mineru_slot(
    ...
    shared_cooldown = SharedCooldown(
        runtime_dir(cfg, "mineru_cooldown", fallback=Path(book.output_dir).parent),
        ttl_seconds=float(cfg.get("runtime", {}).get("cooldown_refresh_seconds", 1.0)),
    )
    slot_dir = _mineru_slot_dir(cfg, book)
    ...
    try:
        while slot_path is None:
            ...
            for _, provider in ranked:
                candidate = _try_take_slot(slot_dir, provider, book, stale_seconds, now)
...
def parse_book_with_mineru(book: BookRecord, cfg: dict[str, Any]) -> MinerUParseResult:
    output_dir = Path(book.output_dir)
    raw_path, parsed_path, image_map_path, status_path = _mineru_paths(book, cfg)
    cache_only = bool(cfg["runtime"].get("cache_only"))
    if cache_only:
        missing = [path for path in (raw_path, parsed_path, image_map_path) if not path.is_file()]
        if missing:
            missing_text = ", ".join(str(path) for path in missing)
            raise FileNotFoundError(
                f"cache-only MinerU files missing for book_id={book.book_id}: {missing_text}"
            )
    if (cache_only or bool(cfg["runtime"]["reuse_mineru"])) and raw_path.exists() and parsed_path.exists() and image_map_path.exists():
        try:
            ...
            status = _validate_mineru_result(book, content, image_map, cfg, "cache")
            write_json(status_path, status)
            ...
            failed_status = _build_mineru_status(book, [], {}, cfg, "cache", f"ignored invalid cache: {exc}")
            write_json(status_path, failed_status)
            if cache_only:
                raise MinerUParseIncompleteError(
                    f"cache-only MinerU cache invalid for book_id={book.book_id}: {exc}"
                ) from exc
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            with _mineru_slot(book, cfg, providers, exclude=tried) as provider:
                tried.add(provider.name)
                payload = MinerUClient(provider.cfg).parse_pdf(Path(book.source_pdf))
            shared_cooldown.clear(provider.name)
            content = find_content_list(payload)
            source = f"mineru_attempt_{attempt}_{provider.name}"
            status = _validate_mineru_result(book, content, {}, cfg, source)
            ...
            failed_status = _build_mineru_status(
                book, [], {}, cfg, f"mineru_attempt_{attempt}_{provider_name}", str(exc)
            )
            try:
                write_json(status_path, failed_status)
            except Exception:
    ...
    raise RuntimeError(f"MinerU parse failed for book_id={book.book_id}: {last_error}") from last_error
