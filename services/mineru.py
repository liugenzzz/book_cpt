...
@contextmanager
def _mineru_slot(book: BookRecord, cfg: dict[str, Any]):
    mineru_cfg = cfg.get("mineru", {})
    ...
    output_root = Path(cfg.get("runtime", {}).get("output_root") or Path(book.output_dir).parent)
...
def parse_book_with_mineru(book: BookRecord, cfg: dict[str, Any]) -> MinerUParseResult:
    ...
    for attempt in range(1, attempts + 1):
        try:
            with _mineru_slot(book, cfg):
                payload = MinerUClient(cfg["mineru"]).parse_pdf(Path(book.source_pdf))
            content = find_content_list(payload)
            status = _validate_mineru_result(book, content, {}, cfg, f"mineru_attempt_{attempt}")
            image_map = save_extracted_images(payload, content, output_dir, cfg)
            status = _build_mineru_status(book, content, image_map, cfg, f"mineru_attempt_{attempt}")
            ...
            last_error = exc
            failed_status = _build_mineru_status(book, [], {}, cfg, f"mineru_attempt_{attempt}", str(exc))
            try:
                write_json(status_path, failed_status)
            except Exception:
                pass
