...
class GenerationResumeTests(unittest.TestCase):
    def test_successful_job_samples_are_appended_before_entire_batch_finishes(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            book = SimpleNamespace(book_id="book-1")
            first_job = SimpleNamespace(task_type="paragraph_summary", book=book)
            second_job = SimpleNamespace(task_type="paragraph_summary", book=book)
            ...
            def fake_generate_for_job(job: object, *_args: object) -> list[dict[str, object]]:
                if job is second_job:
                    ...
                    return [{"id": "second", "task_type": "paragraph_summary"}]
                return [{"id": "first", "task_type": "paragraph_summary"}]
            ...
            with patch.object(pipeline, "generate_for_job", side_effect=fake_generate_for_job):
                ...
                try:
                    ...
                    raw_path = output_dir / "samples/raw/paragraph_summary.jsonl"
    ...
    def test_generation_resumes_from_completed_raw_sample_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state = SimpleNamespace(error=lambda payload: None)
            book = SimpleNamespace(book_id="book-1")
            first_page = SimpleNamespace(chapter_no="1", page_index=1, page_label="1")
            second_page = SimpleNamespace(chapter_no="1", page_index=2, page_label="2")
            first_job = SimpleNamespace(task_type="paragraph_summary", book=book, page=first_page, block=None)
            second_job = SimpleNamespace(task_type="paragraph_summary", book=book, page=second_page, block=None)
            raw_path = output_dir / "samples/raw/paragraph_summary.jsonl"
                ...
                json.dumps(
                    {
                        ...
                        "pipeline_version": "test-pipeline",
                        "completed_jobs": {
                            "paragraph_summary|book-1|1|1|page": {
                                "task_type": "paragraph_summary",
                                "sample_count": 1,
                            }
                        },
            ...
            def fake_generate_for_job(job: object, *_args: object) -> list[dict[str, object]]:
                generated_pages.append(job.page.page_index)  # type: ignore[attr-defined]
                return [
                    {
                        "id": "second",
                        "task_type": "paragraph_summary",
            ...
            self.assertEqual(generated_pages, [2])
