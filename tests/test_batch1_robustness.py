...
from book_cpt.core.io_utils import atomic_write_text, read_json, write_json, write_jsonl
from book_cpt.tasks.dedup import _NearDuplicateIndex, _near_duplicate, deduplicate
...
class DeduplicateIntegrationTests(unittest.TestCase):
    ...
    def _pt_sample(self, sample_id, text, page=1):
        return {
            ...
            "metadata": {"book_id": "j", "chapter_no": "1", "page_index": page, "task_type": "domain_knowledge_corpus"},
        }
    ...
    def test_non_pt_tasks_are_not_near_deduped(self) -> None:
        rows = [
            {"id": "a", "task_type": "paragraph_summary", "question": "q", "answer": "结论基本一致",
             "output_payload": {"summary": "结论基本一致"},
             "metadata": {"book_id": "j", "chapter_no": "1", "page_index": 1, "task_type": "paragraph_summary"}},
            {"id": "b", "task_type": "paragraph_summary", "question": "q2", "answer": "结论基本一致的另一句",
...
class ImageQualityCacheTests(unittest.TestCase):
    def test_repeated_calls_hit_cache(self) -> None:
        from book_cpt.processing import image_quality
...
class GenerationProgressLogTests(unittest.TestCase):
    ...
    def _run(self, job_count, **runtime):
        ...
        book = SimpleNamespace(book_id="b1")
        jobs = [
            SimpleNamespace(task_type="paragraph_summary", book=book,
                            page=SimpleNamespace(page_index=i), block=None)
            for i in range(job_count)
        ]
        logger = logging.getLogger("book_cpt_progress_test")
    ...
    def test_missing_logger_on_state_does_not_crash(self) -> None:
        ...
        book = SimpleNamespace(book_id="b1")
        jobs = [SimpleNamespace(task_type="paragraph_summary", book=book,
                                page=SimpleNamespace(page_index=0), block=None)]
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(pipeline, "generate_for_job", return_value=[{"id": "s", "task_type": "paragraph_summary"}]):
