...
from book_cpt.services.cooldown import SharedCooldown, runtime_dir
from book_cpt.services.mineru import _mineru_slot, mineru_providers


def _book(tmpdir: str, book_id: str = "b1") -> SimpleNamespace:
    return SimpleNamespace(book_id=book_id, output_dir=str(Path(tmpdir) / "out" / book_id))
...
class SlotDispatchTests(unittest.TestCase):
    def test_second_request_goes_to_the_other_instance(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            book = _book(tmp)
            with _mineru_slot(book, cfg) as first:
                with _mineru_slot(book, cfg) as second:
    ...
    def test_slot_is_released_after_use(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            book = _book(tmp)
            with _mineru_slot(book, cfg) as provider:
...
class VlmSharedCooldownTests(unittest.TestCase):
    def test_failure_in_one_pool_is_visible_to_another(self) -> None:
        """两个 VlmPool 实例 = 两个书籍子进程，冷却状态必须互相看见。"""
        from book_cpt.services.clients import VlmPool
        from book_cpt.tests.test_vlm_pool_dispatch import _FakeClient
        from book_cpt.services.clients import _PooledVlmClient
