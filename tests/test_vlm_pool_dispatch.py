...
from book_cpt.services.clients import VlmPool, _PooledVlmClient
...
class QuotaScalingTests(unittest.TestCase):
    def test_quota_is_divided_by_book_workers(self) -> None:
...
class ProviderNameUniquenessTests(unittest.TestCase):
    ...
    def test_shipped_providers_declare_positive_quota(self) -> None:
        ...
        for client in pool.clients:
            self.assertGreaterEqual(client.max_concurrency, 1, client.name)
            self.assertTrue(client.model, f"provider {client.name} 没配 model")
            self.assertTrue(client.cfg.get("url"), f"provider {client.name} 没配 url")
