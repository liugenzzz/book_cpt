...
def _near_duplicate(text: str, seen_texts: list[str], threshold: float) -> bool:
    ...
    for seen in seen_texts:
        ...
        jaccard = overlap / max(1, len(grams | seen_grams))
...
def deduplicate(samples: list[dict[str, Any]], cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    per_page_task: dict[tuple[str, int, str], int] = {}
    ...
    for sample in samples:
        ...
        if isinstance(payload, dict) and payload.get("text"):
            ...
            text_basis = f"{sample.get('question')}||{sample.get('answer')}||{json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str)}"
